...
export const projects: Project[] = [
  ...
  },
  {
    ...
    role: "全栈开发",
    status: "已上线",
    description: "本项目面向公交出行场景，提出一种融合检索增强生成(RAG)与图神经网络(GNN)的大语言模型预测框架。项目通过多维自适应相似度检索乘客历史行为，利用多层图注意力网络(GAT)捕捉站点空间依赖，并在动态锚点蒸馏与信任域约束下微调轻量级大模型，实现高准确率和强可解释性的端到端公交下车站点预测。",
    techStack: ["Python", "大语言模型微调", "图神经网络(GAT)", "检索增强生成(RAG)", "时空数据挖掘", "提示词工程"],
    highlights: [
      "独立完成公交出行数据处理与预测流程设计：负责从公交刷卡记录、车辆 GPS 轨迹、线路站点数据中提取有效特征，完成异常数据清洗、时间字段标准化、线路方向识别、站点序列整理和乘客出行记录构建，为后续模型训练和预测提供统一的数据输入。",
      "设计乘客上车站点匹配与下车候选站点召回机制：基于刷卡时间、车辆编号、线路方向和车辆进出站时间窗口，推算乘客上车站点；针对下车站点缺失问题，结合乘客历史出行行为、同线路站点序列、相似乘客记录和全局 OD 分布，构建多层候选召回策略，提升真实下车站点进入候选集合的概率。",
      "引入图增强模型刻画公交站点空间关系：将公交站点抽象为图结构，站点作为节点，线路相邻关系、地理邻近关系和换乘关系作为边，利用图增强思想提取站点之间的空间依赖特征，使模型不只依赖历史统计频次，还能理解站点之间的拓扑关系和出行可达性。",
      "构建候选排序模型，重点优化 Top1 预测效果：针对“候选站点能召回，但最终 Top1 不够准”的问题，设计候选站点排序流程，将时间特征、空间距离、历史频次、相似乘客行为、线路顺序、图增强特征等作为输入，对 Top-K 候选进行二次排序，重点提升最终预测站点的准确性。",
      "封装预测接口与系统展示页面：独立完成算法流程封装和系统页面展示，将数据处理、候选召回、模型排序、预测结果输出和实验指标展示整合到项目中。前端页面可展示项目介绍、技术架构、预测流程、实验结果和项目入口，方便企业或导师快速查看项目成果。",
      "完成实验评估与结果分析：对模型预测结果进行评估，关注候选召回率、Top1 准确率、Top3 命中率、Top-K 召回率、解析成功率和兜底比例等指标。通过对失败样本进行分析，进一步优化候选召回策略和排序模型特征设计。"
    ],
