...
export const projects: Project[] = [
  ...
  },
  {
    ...
    name: "基于图增强模型的公交下车站点预测系统 (Graph-enhanced Transit Dispatch System)",
    period: "2025.10 - 至今",
    role: "全栈开发",
    status: "已上线",
    ...
    techStack: ["图增强模型", "检索增强", "候选召回", "大模型推理", "公交OD预测"],
    highlights: [
      "独立完成数据处理与候选召回流程：完成公交刷卡记录、车辆 GPS 轨迹、线路站点数据的清洗与整理，基于刷卡时间、车辆编号和站点序列推算乘客上车站点，并结合历史出行行为生成候选下车站点。",
      "引入图增强模型提升站点关系表达能力：将公交站点构建为图结构，融合线路相邻关系、地理邻近关系和换乘关系，使模型能够理解站点之间的空间拓扑关系，提升下车站点预测效果。",
      "构建排序与大模型推理流程：围绕 Top-K 候选站点进行二次排序，结合乘客历史规律、相似乘客行为、时间特征和图增强特征，优化最终 Top1 下车站点预测结果，并封装为可在线展示的系统。"
    ],
